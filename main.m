...
datanames = {'handwritten-5view'};  % this is a unit test datasets list
...
lamda1 = [1e-1, 1e1];  % this is a unit test ratio list
lamda2 = [1e-1, 1e1];  
...
for idata = 1:length(datanames)  % traverse dataset
    ...
    for perMising = 1:length(del)  % traverse validation ratios
        ...
        result = runAlgorithm(datasetName, ratio, datadir, lamda1, lamda2, readmeFile);
        ...
        % Append top results to readme  
        fileID = fopen(readmeFile, 'r');  
        existingContent = fread(fileID, '*char')';  
        fclose(fileID);  
        fileID = fopen(readmeFile, 'w');  
