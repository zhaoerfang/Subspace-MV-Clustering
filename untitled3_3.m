clear;clc;
addpath('measure','tools');

resultdir = 'Results3_3/';
if (~exist('Results3_3', 'file'))
    mkdir('Results3_3');
    addpath(genpath('Results3_3/'));
end
...
for idata=1:length(dataname)
    ...
    for perMising=1:length(del)
        ...
        lamda1=[1e-15,1e-13,1e-11,1e-9,1e-7,1e-5,1e-4,1e-3,0.01:0.01:0.2,1e1,1e3,1e5,1e6,1e7,1e8,1e9,1e10,1e11,1e12,1e13,1e14,1e15];
        ...
        for i=1:length(lamda1)
            %for j=1:length(lamda2)
                ...
                disp([char(dataname(idata)),'_Per',num2str(del(perMising)),'_fold',num2str(f), ' lamda1=', num2str(lamda1(i))]);
                
                    %% train 2,1-norm  + tensor 
                    ...
                    while (Isconverg == 0)
                        ...
                        for iz=1:numview
                            ...
                            Z0{iz}=((2*beta(iz)+mu2)*eye(np(iz))+mu1*A{iz}'*X{iz}'*X{iz}*A{iz})\(A{iz}'*(mu1*X{iz}'*X{iz}+2*beta(iz)*U+mu2*G{iz}-W{iz})*A{iz}+A{iz}'*X{iz}'*(Y{iz}-mu2*E0{iz}));
                        ...
                        [Econcat] = solve_l1l2(F,lamda1(i)/mu1);
                        ...
                        %-------------------5 update tensor_G--------------------------------
                        w=tensor_W(:);
                        [g, objv] = wshrinkObj(z + 1/mu2*w,1/mu2,sx,0,1);
                    ...
                    Res=[Res;f,lamda1(i),res];
                    ResBest{f}=[ResBest{f};f,lamda1(i),res];
                    %meaRes=mean(Res)
