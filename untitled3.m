...
datadir='datasets/';
dataname = {'ORL-3view'};
%dataname = {'ORL-3view','bbcsport-4view','3sources-3view','BDGP_4view'};
del=[0.1];
%del=[0.1,0.3,0.5,0.7];
%del=[0.1,0.3,0.5];
    ...
    for perMising=1:length(del)
        ...
        for i=1:length(lamda1)
            for j=1:length(lamda2)
                ...
                %% train 2,1-norm + hyperlaplace + tensor + 完整Z做hyperlaplace
                ...
                while (Isconverg == 0)
                    ...
                    for iz=1:numview
                        ...
                        Z0{iz}=((2*beta(iz)+mu1+mu3)*eye(np(iz))+mu2*A{iz}'*X{iz}'*X{iz}*A{iz})\(A{iz}'*(mu2*X{iz}'*X{iz}+2*beta(iz)*U+mu1*Q{iz}-M{iz}+mu3*G{iz}-W{iz})*A{iz}+A{iz}'*X{iz}'*(Y{iz}-mu2*E0{iz}));
                    end
                    %-------------------2 update U--------------------------------
                    sumU=0; 
                    for iu=1:numview
                    ...
                    %-------------------3 update tensor_Z--------------------------------
                    for iiz=1:numview
                        Z{iiz}=A{iiz}*(Z0{iiz}-A{iiz}'*U*A{iiz})*A{iiz}'+U;
                        tensor_Z(:,:,iiz)=Z{iiz};
                    end
                    z=tensor_Z(:);
                    %-------------------4 update E--------------------------------
                    ...
                    %-------------------5 update Q--------------------------------
                    for iq=1:numview
                        Q{iq}=(mu1*Z{iq}+M{iq})/(mu1*eye(numsample)+2*lamda2(j)*L{iq});
                    end
                    %-------------------6 update tensor_G--------------------------------
                    w=tensor_W(:);
        ...
        save([resultdir, char(dataname(idata)),'_per',num2str(del(perMising)),'_',num2str(f),'_result.mat'], "ResBest","ResStd");
        end
    end
end
